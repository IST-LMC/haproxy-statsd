...
def get_haproxy_report(socket_ip, socket_port=9898, user=None, password=None):
  stats_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
  stats_sock.settimeout(5)
  stats_sock.connect((socket_ip, int(socket_port)))
  stats_sock.sendall('show stat\n')
  stats = stats_sock.recv(4096)
  stats_sock.close()
  data = stats.lstrip('# ')
  return csv.DictReader(data.splitlines())

def report_to_statsd(stat_rows, host=os.getenv('STATSD_HOST', '127.0.0.1'), port=os.getenv('STATSD_PORT', 8125), namespace=os.getenv('STATSD_NAMESPACE', 'haproxy')): 
  ...
  for row in stat_rows:
    ...
    for stat in ['scur', 'smax', 'ereq', 'econ', 'rate', 'bin', 'bout', 'hrsp_1xx', 'hrsp_2xx', 'hrsp_3xx', 'hrsp_4xx', 'hrsp_5xx', 'qtime', 'ctime', 'rtime', 'ttime']:
      ...
      print '%s.%s:%s|g' % (path, stat, val), (host, port)
      stat_count += 1
  return stat_count


if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Report haproxy stats to statsd')
  parser.add_argument('-c', '--config', help='Config file location', default='./haproxy-statsd.conf')
  parser.add_argument('-1', '--once', action='store_true' ,help='Run once and exit', default=False)

  args = parser.parse_args()
  config = ConfigParser.ConfigParser({
    'haproxy_url': os.getenv('HAPROXY_SOCKET_IP', '127.0.0.1'),
    'haproxy_url': os.getenv('HAPROXY_SOCKET_PORT', '9898'),
  ...
  try:
    while True:
      report_data = get_haproxy_report(
        config.get('haproxy-statsd', 'haproxy_socket_ip'),
        config.get('haproxy-statsd', 'haproxy_socket_port'),
